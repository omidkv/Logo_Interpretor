...
type Global = (Frame, Bool, ManyLocal)
...
myDisplay = do
  ...
  draw $ (Join (Bend 90) (Join (fst $ runState (found (cadddr (p test))) initialState) (drawTurtle 10)))
  flush
...
found' (Cons (Symbol fname) args)  = do
  ...
  case alpha of 
    Closure params body local' -> do 
                                  ...
                                  put(env, penState, local')
                                  Join <$> return(Err ("this is newArgs"++ (show l))) <*> (Join <$> found' body <*> removeLayer )
          
    _ -> return (Err $ "This failed to create" ++  (show (args')))
...
found (Cons (Symbol fname) args) = do
  ...
  case alpha of 
    ...
    _ -> return (Err ("This was fname"++fname))
found (Cons x y) = Join <$> found x <*> found y
