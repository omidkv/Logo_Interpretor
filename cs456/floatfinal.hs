...
s1 = "(1 2 3)"
s2 = "(define fact (lambda (x) (if (= x 0) 1 (* x (fact (- x 1.399))))))"
...
ex9Test = "(repeat 5 (forward 6.5) (right (/ 360 5))) "
...
myDisplay = do
  ...
  --draw $ Join (Bend 90) (Join (Join (Join (Bend (-45)) (Straight 1)) (Join (Bend (-90)) (Straight 1))) (Join (Bend (-135)) (Straight 1.4)))
  --draw $ (Join (Join (Bend 90) (fst $ runState (found (cadddr (p test))) initialState)) (drawTurtle 10))
  draw $ (Join (Join (Bend 90) (fst $ runState (found (p ex9Test)) initialState)) (drawTurtle 0.5))
  flush
...
body' (Cons (Symbol "to") (Cons (Symbol name) (Cons args bod))) = bod
