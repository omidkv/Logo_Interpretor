...
data Sexpr = Symbol String | Number GLfloat | FloatS GLfloat | Boolean Bool | Nil | Cons {car :: Sexpr, cdr :: Sexpr} | Closure {args :: [String], body :: Sexpr, env :: Local} | Unary {name :: String, func1 :: Sexpr -> Sexpr} | Binary {name :: String, func2 :: Sexpr -> Sexpr -> Sexpr} | Void
...
test5 = "(define fancy-spiral '((to fancy-spiral (size angle) (if (> size 1) (stop) ) (color (* size (/ 360 200))) (forward size) (right angle) (fancy-spiral (+ size 1) angle) ) (penup) (forward 120) (pendown) (fancy-spiral 0 91) ))"
...
test7 = "(define lissajous '((to lissajous (a b c t) (penup) (setxy (* (cos c) 75) 100) (pendown) (repeat 1 (color t) (setxy (* (cos (+ (* t a) c)) 75) (+ (* (sin (* t b)) 75) 100)) (make t (+ t 1)) ) ) (lissajous 0.1396 -0.12215 0.2094 0)))"
...
testSetXY (theta, dx, dy) = Join (Join (Join (Join (Bend (theta)) (Invisible dy)) (Bend (-90))) (Invisible dx)) (Bend (90-theta))
...
trackXY w (x1, y1)  = (x2, y2)
  where x2 = x1 + ((cos (degToRad w)) * l)
        y2 = y1 + ((sin (degToRad w)) * l)

initialTurtle = (0,0,90)

initialState = ([], True,[],initialTurtle,False,initialTurtle)
myDisplay = do
  ...
  draw $ (Join (Bend 90) (Join (fst $ runState (found (cadddr (p test7))) initialState) (drawTurtle 5)))
  flush
...
found' (Cons (Symbol "setxy") value) = do
    (envState, penState,local,_,_,_) <- get
    let y = evalProc value (head local)

    return (Err $"This is the setxy" ++(show value))
...
found' (Cons (Symbol "make") (Cons (Symbol arg) (Cons expr Nil))) = do
  ...
  return(Err ("Make Found" ++ (show newLocal)))
...
found' (Cons (Symbol "penup") Nil) = do {(envState, _,local,lastTurtle,stop,lastPush) <- get; put (envState, False,local,lastTurtle,stop,lastPush); return (Invisible 0)}
found' (Cons (Symbol "pendown") Nil) = do {(envState, _,local,lastTurtle,stop,lastPush) <- get; put (envState, True,local,lastTurtle,stop,lastPush); return (Straight 0)}
...
evalProc (Cons (Cons (Symbol oper) (Cons v Nil))Nil) = 
                                      case oneArg oper of
                                Just f -> (f v)
                                Nothing -> undefined
                                 

evalProc (Cons (Cons (Symbol oper) (Cons value1 value2)) Nil) = 
                                        case op oper of
                                            Just f -> Just value -> (f (evalProc value1 local) (evalProc value2 local))
                                            Nothing -> undefined

evalProc (Cons (Symbol oper) (Cons value1 value2)) = 
                                          case op oper of
                                            Just f -> Just value -> (f (evalProc value1 local) (evalProc value2 local))
                                            Nothing -> undefined
...
found (Cons (Symbol "forward") value) = do
  ...
  (envState, penState,_,currentTurt,stop,pushTurt) <- get
  case stop of
    False -> case penState of 
              True -> return $ (Straight (y))
              False -> return $ (Invisible (y))
